...
import { isAdminLogged, adminLoginWithPass } from "@/services/UsersService";
...
export function AdminAuth({ className, ...props }: UserAuthFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      setLoading(true);
      await adminLoginWithPass(values.email, values.password);
      setLoading(false);
      window.location.href = "/admin/dashboard";
    } catch (err: any) {
      setLoading(false);
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className="grid gap-2">
              ...
              render={({ field }) => {
                ...
                return (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="אימייל" type="text" {...field} />
                    </FormControl>
                  </FormItem>
                );
              }}
              ...
              render={({ field }) => {
                ...
                return (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="סיסמה" type="password" {...field} />
                    </FormControl>
                  </FormItem>
                );
              }}
            />
            {loading ? (
              <Button disabled className="bg-red-300">
